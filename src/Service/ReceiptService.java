package Service;

import API.IReceiptService;
import DAO.ReceiptDao;
import VO.ReceiptVO;
import VO.UserVO;
import VO.WarehouseVO;

import java.io.*;
import java.text.SimpleDateFormat;
import java.util.*;
import java.util.List;

// 오류는 나중에 Exception 처리 클래스 만들어서 따로 처리 일단은 Exception 에 다 던져 놓기

public class ReceiptService implements IReceiptService {

    ReceiptDao receiptDao;
    ReceiptVO receipt;
    WarehouseVO warehouseVO;


    List<ReceiptVO> receiptVOList = new ArrayList<>();
    BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
    static int approval = 0;

    // -----------------------------------------------------관리자
    /* 총관리자가 이용하는 입고 요청 메뉴
    1. 입고 요청 승인
    2. 입고 요청 수정 -> 입고 요청 지정, 입고 날짜 지정
    3. 입고 요청 취소
    4. 입고 고지서 출력
    5. 입고 현황 조회 (기간별, 월별, QR 코드)
    6. 로그 아웃(뒤로 가기?)
     */

    public void receiptAdminMenu(UserVO userVO) {
        System.out.println("관리자 전용 입고 관리 메뉴입니다.");
        System.out.println("1. 입고 요청 승인 | 2. 입고 요청 수정 | 3. 입고 요청 취소\n" +
                "4. 입고 고지서 출력 | 5. 입고 현황 조회 | 6. 로그아웃");

        System.out.print("선택 : ");
        try {
            String choice = input.readLine();

            switch (choice) {
                case "1" -> receiptApprove(userVO);
                case "2" -> receiptRequestUpdate(userVO);
                case "3" -> receiptRequestDelete(userVO);
                case "4" -> receiptPaper(userVO);
                case "5" -> receiptListChoice(userVO);
//            case "6" -> userService.memberLogin();
            }
        } catch (Exception e) {
        }
    }

    // 입고 요청 승인 화면
    public void receiptApprove(UserVO userVO) throws Exception {
        System.out.println("입고 요청 승인 화면입니다.");
        receiptDao.receiptReadApproval();           // -> approval 값이 0인 얘들 리스트
        System.out.print("승인할 입고 요청을 입력하세요.\nq!를 입력하면 입력이 중지됩니다.");

        while (true) {
            System.out.print("입력 : ");
            String choice = input.readLine();
            if (choice.equals("q!")) {
                break;
            }
            for (ReceiptVO receiptVO : receiptVOList) {
                if (receiptVO.getUId() == Integer.parseInt(choice)) {
                    receiptVO.setApproval(1);
                    receiptVO.setState(1);
//                    receiptVO.setQrCode(1);       qr 코드 세팅 어디다가 해야됨?
//                    qrCodeInsert(receiptVO);

                    // 상태도 변경
                    // qr 코드도 변경
                }
            }
        }
        System.out.println("입고 요청이 승인되었습니다.");
        receiptAdminMenu(userVO);
    }

    // qr 코드 생성화면 ------------------------------------------------------------------------------------------------------------------------


    // QR 바코드 조회 화면
//    private void qrCodeList(UserVO userVO) throws Exception {
//        System.out.println(userVO.getID() + "계정의 입고 승인된 건의 qr 코드 목록");
//        for (ReceiptVO receiptVO : receiptVOList) {
//            if (receiptVO.getApproval() == 1) {
//                receiptDao.receiptRead(userVO);
//            }
//        }
//
//        while (true) {
//            System.out.print("조회하고 싶은 번호를 선택하세요 : ");
//            String choice = input.readLine();
//            for (ReceiptVO receiptVO : receiptVOList) {
//                if (receiptVO.getReceiptId() == Integer.parseInt(choice)) {
//
//                }
//            }
//        }
//
//    }
    //------------------------------------------------------------------------------------------------------------------------

    // 입고 고지서 출력
    /*
        총관리자 기능
        입고 승인된 / approval = 1인 것들
        회원들의 입고 요청 승인된 것들
        리스트 출력해서 선택하면 그 입고 고지서가 출력
     */
    private void receiptPaper(UserVO userVO) throws Exception {
        System.out.println("=====입고 고지서 조회=====");
        while (true) {
            receiptList(userVO);    // 관리자로 들어가서 회원들 입고 요청 승인된 리스트 출력
            System.out.print("출력할 입고 건을 선택하세요\n UID 선택 : ");
            String choice = input.readLine();
            for (ReceiptVO receiptVO : receiptVOList) {
                if (receiptVO.getUId() == Integer.parseInt(choice)) {
                    System.out.println("=====입고 고지서 출력=====");
                    System.out.printf("입고 번호 : %d\n", receipt.getReceiptId());
                    System.out.printf("입고 날짜 : %s\n", receipt.getReceiptDate());
                    System.out.printf("상품 ID : %s, 상품 수량 : %s\n", receipt.getPId(), receipt.getProductQuantity());
                    System.out.printf("입고될 창고 ID : %d\n", receipt.getWId());
                    System.out.printf("상태 : %d\n", receipt.getState());
                }
            }
            System.out.print("더 출력하시겠습니까? (y/n) : ");
            String s = input.readLine();

            if (s.equalsIgnoreCase("n"))
                break;

        }

    }
    // ----------------------------------------------------- 회원

    /* 회원이 이용하는 입고 요청 메뉴
    1. 입고 요청
    2. 입고 요청 수정
    3. 입고 요청 취소
    4. 입고 요청 조회 (기간별, 월별, QR 코드)
    5. 로그 아웃 ( 뒤로 가기?)
    */

    public void receiptUserMenu(UserVO userVO)  {
        System.out.println("회원 입고 관리 메뉴입니다.");
        System.out.println("1. 입고 요청 | 2. 입고 요청 수정 | 3. 입고 요청 취소" +
                "\n4. 입고 요청 조회 | 5. 로그아웃");
        System.out.print("선택 : ");
        try {
            String choice = String.valueOf(input.readLine());

            switch (choice) {
                case "1" -> receiptRequest(userVO);
                case "2" -> receiptRequestUpdate(userVO);
                case "3" -> receiptRequestDelete(userVO);
                case "4" -> receiptListChoice(userVO);
//            case "5" -> userService.memberLogin();
            }
        } catch (Exception e) {}
    }

    // 입고 요청 화면 --> User 만 이용
    private void receiptRequest(UserVO userVO) throws Exception {

        System.out.println("회원 전용 입고 요청 화면입니다.");


        System.out.println("상품의 ID를 입력해주세요.");
        int productId = Integer.parseInt(input.readLine());

        System.out.println("입력한 상품의 수량을 입력해주세요.");
        int productQuantity = Integer.parseInt(input.readLine());

        System.out.println("창고 종류를 선택하세요.");
        System.out.println("1. 기본 | 2. 냉장 | 3. 냉동");
        int warehouseId = Integer.parseInt(input.readLine());

        receipt = new ReceiptVO(getCurrentDate(),
                productQuantity, userVO.getUserID(), productId, warehouseId, approval);
        receiptDao.receiptCreate(receipt);
        receiptUserMenu(userVO);

        System.out.println("입고 요청되었습니다.");

    }


    // 입고 요청 수정 화면 -> 입고 위치 지정, 입고 날짜 지정 같이
    private void receiptRequestUpdate(UserVO userVO) throws Exception {
        System.out.println("회원 전용 입고 요청 수정 화면입니다.");

        for (ReceiptVO receiptVO : receiptVOList) {
            if (receiptVO.getUId() == userVO.getUserID()) {

                System.out.println("상품의 ID를 입력해주세요.");
                receiptVO.setPId(Integer.parseInt(input.readLine()));

                System.out.println("입력한 상품의 수량을 입력해주세요.");
                receiptVO.setProductQuantity(Integer.parseInt(input.readLine()));

                System.out.println("창고를 선택하세요.");
                System.out.println("1. 기본 | 2. 냉장 | 3. 냉동");
                receiptVO.setWId(Integer.parseInt(input.readLine()));

                receiptDao.receiptUpdate(receipt);
                receiptUserMenu(userVO);

            }
        }
        System.out.println("수정 되었습니다.");
    }

    // 입고 요청 취소 화면
    private void receiptRequestDelete(UserVO userVO) throws Exception {
        System.out.println("회원 전용 입고 요청 삭제 화면입니다.");

        System.out.println("입고 요청을 삭제하시겠습니까? (y/n)?");

        if (input.readLine().equalsIgnoreCase("y")) {
            for (ReceiptVO receiptVO : receiptVOList) {
                if (receiptVO.getUId() == userVO.getUserID()) {
                    receiptVOList.remove(receiptVO);
                    receiptDao.receiptDelete(receiptVO);
                }
            }
            System.out.println("회원님의 입고 요청이 삭제되었습니다.");
            receiptUserMenu(userVO);

        } else if (input.readLine().equalsIgnoreCase("n")) {
            System.out.println("회원님의 입고 요청이 삭제되지 않았습니다.");
            receiptUserMenu(userVO);

        } else
            System.out.println("입력 오류");


    }

    //----------------------------------------현황 조회 선택 화면
    // 회원일 경우
    // 1. 입고 현황 조회
    // 2. 기간별 입고 현황
    // 3. 월별 입고 현황

    // 관리자일 경우
    // 4. qr 코드 조회  추가

    public void receiptListChoice(UserVO userVO) throws Exception {
        System.out.println("입고 현황 조회 선택 화면");
        System.out.println("1. 입고 현황 조회 | 2. 기간별 입고 현황" +
                "\n3. 월별 입고 현황 | 4. 뒤로 가기");
        if (userVO.getUserType() == 1) {
            System.out.println("5. QR 코드 조회");
        }

        System.out.print("선택 : ");
        String choice = String.valueOf(input.readLine());

        switch (choice) {
            case "1" -> receiptList(userVO);
            case "2" -> receiptPeriodicList(userVO);
            case "3" -> receiptMonthList(userVO);
            case "4" -> {
                if (userVO.getUserType() == 1)   // admin 계정
                    receiptAdminMenu(userVO);
                else
                    receiptUserMenu(userVO);
            }
//            case "5" -> qrCodeList(userVO);
        }
    }

    // 입고 현황 조회 화면 -> 회원일 때랑 관리자일 때 다르게
    private void receiptList(UserVO userVO) {
        for (ReceiptVO receiptVO : receiptVOList) {
            if (receiptVO.getUId() == userVO.getUserID()) {
                if (userVO.getUserType() == 1) {
                    System.out.println(userVO.getID() + " 관리자 계정의 입고 현황 리스트입니다.");
                    receiptDao.receiptRead(userVO);
                } else {
                    System.out.println(userVO.getID() + " 계정의 입고 현황 리스트입니다.");
                    receiptDao.receiptRead(userVO);
                }
            }
        }
    }

    // 기간별 입고 현황 화면 -> 회원일 때랑 관리자일 때 다르게
    private void receiptPeriodicList(UserVO userVO) throws Exception {
        System.out.println("기간 별 입고 현황 입니다.");
        System.out.println("조회할 기간을 입력해주세요.");
        System.out.print("시작 날짜 : ");
        String firstDate = input.readLine();
        System.out.print("끝 날짜 : ");
        String lastDate = input.readLine();
        System.out.println(firstDate + " ~ " + lastDate + "기간의 입고 현황입니다.\n");
        receiptDao.receiptPeriodRead(userVO, firstDate, lastDate);
        receiptListChoice(userVO);
    }

    // 월별 입고 현황 조회 화면 -> 회원일 때랑 관리자일 때 다르게
    private void receiptMonthList(UserVO userVO) throws Exception {
        System.out.println(userVO.getUserName() + "의 월 별 입고 현황입니다.");
        receiptDao.receiptMonthRead(userVO);
        receiptListChoice(userVO);
    }


    private static String getCurrentDate() {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        return dateFormat.format(new Date());
    }
}